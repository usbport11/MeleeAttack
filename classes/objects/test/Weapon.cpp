...
MWeapon::MWeapon():MPhysicQuad() {
	...
	InProcess = false;
	StopByWall = false;
	SwingAngle = 45 * PH_DEGTORAD;
}

bool MWeapon::GetInProcess() {
	return InProcess;
}

float MWeapon::GetSwingAnle() {
	return SwingAnle;
}

void MWeapon::SetSwingAngle(float Value) {
	SwingAngle = Value;
}
...
void MWeapon::BeginAttack(float inAttackAngle) {
	if(!CanRestart && InProcess) return;
	AttackAngle = inAttackAngle;
	DamagedQuads.clear();
	InProcess = true;
}
...
void Update() {
	if(InProcess && Body->GetAngle() >= AttackAngle - ShiftAngle) EndAttack();
	MPhysicQuad::Update();
}
